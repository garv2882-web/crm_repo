...
export default function DealsPage() {
  ...
  const DEAL_STAGES = settings.dealStages || [];

  const handleCreateDeal = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!userPermissions.canCreateDeals) return;

    try {
      const selectedLead = leads.find(l => l.lead_id === newDeal.lead_id);
      const dataToSubmit = {
        ...newDeal,
        deal_stage: newDeal.deal_stage || (DEAL_STAGES[0]?.id || 'Qualification'),
        deal_value: parseFloat(newDeal.deal_value) || 0,
        probability_percentage: parseFloat(newDeal.probability_percentage) || 50,
        deal_owner: selectedLead?.assigned_to || currentUser?.user_id || ''
      };
  ...
  const canEditDeal = (deal: Deal) => {
    if (userPermissions.canEditAllDeals) return true;
    if (userPermissions.canEditOwnDeals && deal.deal_owner === currentUser?.user_id) return true;
    return false;
  };
  ...
  const moveDealStage = async (deal: Deal, direction: 'forward' | 'backward') => {
    ...
    const currentIdx = stagesIds.indexOf(deal.deal_stage);
    let nextIdx = direction === 'forward' ? currentIdx + 1 : currentIdx - 1;
    if (nextIdx < 0 || nextIdx >= stagesIds.length) return;

    const nextStage = stagesIds[nextIdx];
  ...
  const viewableDeals = userPermissions.canViewAllDeals 
    ? deals 
    : deals.filter(d => d.deal_owner === currentUser?.user_id);

  const filteredDeals = viewableDeals.filter(d => 
  ...
  const dealsByStage = DEAL_STAGES.reduce((acc, stage) => {
    acc[stage.id] = filteredDeals.filter(d => d.deal_stage === stage.id);
    return acc;
  ...
  return (
    <div className="deals-workspace animate-fade-in" style={{ paddingBottom: '40px' }}>
      ...
      <div style={{
        ...
        {userPermissions.canCreateDeals && (
          ...
          return (
            ...
            >
              ...
              <div style={{
                ...
                <div>
                  <span style={{ fontWeight: 700, fontSize: '13px', display: 'block', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', maxWidth: '160px' }} title={stage.name}>
                    {stage.name}
                  </span>
                  ...
                  stageDeals.map(d => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Interactive Move Arrows (guarded by edit/change stage permissions) */}
                        {editable && (
                          <div style={{
                            display: 'flex',
                            justifyContent: 'space-between',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  ...
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={e => setNewDeal({ ...newDeal, deal_stage: e.target.value })}
                      style={{ width: '100%', height: '38px', padding: '0 12px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)' }}
                    >
                      {DEAL_STAGES.map(st => (
