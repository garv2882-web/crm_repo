  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO companies (company_name, company_code, industry, website, country, state, city, annual_revenue, notes, created_by) 
       ...
       RETURNING *`,
      [company_name, company_code, industry, website, country, state, city, annual_revenue || 0, notes, created_by || null]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE companies 
       ...
       RETURNING *`,
      [company_name, company_code, industry, website, country, state, city, annual_revenue, notes, id]
    );
